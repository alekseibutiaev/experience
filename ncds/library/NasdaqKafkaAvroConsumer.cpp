...
NasdaqKafkaAvroConsumer::NasdaqKafkaAvroConsumer(RdKafka::Conf* kafka_cfg,
  ...
  if (kafka_cfg != nullptr) {
    ...
    std::string errstr;
    kafka_conf->set("oauthbearer_token_refresh_cb", token_cb.get(), errstr);
...
std::unique_ptr<RdKafka::KafkaConsumer> NasdaqKafkaAvroConsumer::get_consumer(const std::string &stream_name)
{
  ...
  std::string group_id = client_id + "_" + stream_name + "_" + std::to_string(tPtr->tm_mday);
  if (kafka_conf->set(KafkaConfigLoader::GROUP_ID_CONFIG, group_id.c_str(), errstr) != RdKafka::Conf::CONF_OK)
    throw std::runtime_error("Error setting group id: " + errstr);

  auto kafka_consumer = std::unique_ptr<RdKafka::KafkaConsumer>(RdKafka::KafkaConsumer::create(kafka_conf, errstr));
...
std::set<std::string> NasdaqKafkaAvroConsumer::get_topics() {
    return read_schema_topic.get_topics();
}
