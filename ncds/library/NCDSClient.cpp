...
//    originally returns a string but we're changing to to return a schema object for now
    ...
    std::vector<avro::GenericRecord>
    NCDSClient::top_messages(const std::string &topic_name, long timestamp, int num_messages) {
    ...
    avro::GenericRecord
    NCDSClient::get_sample_messages(const std::string &topic_name, const std::string &message_name, bool all_messages) {
        std::unique_ptr<RdKafka::KafkaConsumer> kafka_consumer = NCDSClient::NCDSKafkaConsumer(topic_name);
